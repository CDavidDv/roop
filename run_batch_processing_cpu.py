...
Versión CPU - evita problemas de CUDA
"""
...
# Configurar variables de entorno para CPU ANTES de cualquier import
...
# Desactivar predictor NSFW para evitar errores
import roop.predictor
def predict_video_skip_nsfw(target_path: str) -> bool:
    print("⚠️ Saltando verificación NSFW para optimizar rendimiento...")
    return False
...
def check_system_info():
    ...
    try:
        ...
        print(f"✅ RAM: {memory.total // (1024**3)}GB total, {memory.available // (1024**3)}GB disponible")
...
def optimize_system_for_cpu_processing():
    """Optimizar sistema para procesamiento CPU"""
    print("🔧 Optimizando sistema para procesamiento CPU...")
    ...
    print("✅ Configuración CPU optimizada")
...
def process_single_video(source_path: str, target_path: str, output_path: str, 
    ...
    cmd = [
        ...
        '--execution-provider', 'cpu'  # Solo CPU
    ]
    ...
    try:
        ...
        print(f"⚙️ Configuración: {execution_threads} hilos, {max_memory}GB RAM, {gpu_memory_wait}s wait")
        ...
        for line in process.stdout:
            ...
            if line:
                ...
                if any(keyword in line for keyword in [
                    ...
                    'Face-Swapper', 'Face-Enhancer', 'ROOP.CORE', 'CPU'
                ]):
                    print(f"  📈 {line}")
...
def main():
    parser = argparse.ArgumentParser(description='Procesar múltiples videos con ROOP (versión CPU)')
